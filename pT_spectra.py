...
def MakeHistos(proc, vals):
  ...
  for iev, event in enumerate(reader):
     ...
     # bs = list(filter(lambda x: abs(x.pdgid)== 5, event.particles))
      ...
      h_myh.Fill(np.sqrt((ys[0].px)**2 + (ys[0].py)**2))
      #h_p    = np.sqrt(hs[0].energy**2-hs[0].mass**2)
      #ctheta = hs[0].pz/h_p

      #h_ctheta.Fill(ctheta)
      sum=(np.sqrt((ys[0].px)**2 + (ys[0].py)**2))
      print(sum)
...
def MakePlot(procs,log=True,ofile="test.png"):
  ...
  if(log):
    ...
    ax.set_ylim([0.,30])
  ax.set_xlim([0,1000])
  plt.xlabel("$p_{T,Y}$",horizontalalignment='left', x=1.0)
...
def main():
  ...
  MakePlot(procs,log=False,ofile="pT_Y_spectrum.png")
  #cwd = os.getcwd()
