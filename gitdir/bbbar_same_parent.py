...
def MakeHistos(proc, vals):
  ...
  h_myh     = ROOT.TH1D('h_myh_{0}'.format(proc), ';m(#gamma, h) [GeV];Events;',100, 124.96,125.06)#4.21398*2, 4.21341*2)
  ...
  for iev, event in enumerate(reader):
     ...
     # bs = list(filter(lambda x: abs(x.pdgid)== 5, event.particles))
      ...
      bbars = list(filter(lambda x: abs(x.parent)== 3, bbars))
      ...
      #print(bs[0].p4().mass)
      h_myh.Fill((bs[0].p4() + bbars[0].p4()).mass )
      
      #print('sum',(bs[0].p4() + bbars[0].p4()).mass )
      sum=(bs[0].p4() + bbars[0].p4()).mass
      print(sum)
      if (sum>124.96) and (sum<125.06):
        bin+=1
...
def main():
  ...
  MakePlot(procs,log=False,ofile="bbar_parent_3.png")
  #cwd = os.getcwd()
